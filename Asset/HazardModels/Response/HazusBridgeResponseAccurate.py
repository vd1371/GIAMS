...
class HazusBridgeResponseAccurate(BaseResponse):
	...
	def get_response(self, previous_condition = -1, soil_type = 'A', pga=None, pgd=0, sa_long=0.5, sa_short = 0.1):
		...
		sa_short = sa_short * short_period_modifier(sa_short, soil_type)
		pga = pga * short_period_modifier(sa_short, soil_type)

		k_shape = 2.5 * sa_long / sa_short
		i_shape = i_shape_calculator(self.bridge_type)
		factor_slight = 1 if i_shape == 0  else min(1, k_shape)

		damage_state_dic = deepcopy(self.damage_state_dic)
		damage_state_dic['Slight'][MEDIAN] = self.damage_state_dic['Slight'][MEDIAN] * factor_slight
		damage_state_dic['Moderate'][MEDIAN] = self.damage_state_dic['Moderate'][MEDIAN] * self.k_skew * self.k_3d
		damage_state_dic['Extensive'][MEDIAN] = self.damage_state_dic['Extensive'][MEDIAN] * self.k_skew * self.k_3d
		damage_state_dic['Collapse'][MEDIAN] = self.damage_state_dic['Collapse'][MEDIAN] * self.k_skew * self.k_3d


		states = [previous_condition]
		for key, val in damage_state_dic.items():
			states.append(val[-1])
		...
		if sa_long == 0:
			...
			# Page 305 HAZUS
			for ds in damage_state_dic.keys():
				prob = norm.cdf(1/damage_state_dic[ds][BETA]*np.log(sa_long/damage_state_dic[ds][MEDIAN]))
